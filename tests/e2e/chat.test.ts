...
test.describe("Chat Page", () => {
  test("page title is scanner-specific", async ({ page }) => {
    await page.goto("/");
    await expect(page).toHaveTitle("CodeGate Guardian");
  });
  ...
  test("shows scanning-first empty state copy", async ({ page }) => {
    await page.goto("/");
    await expect(page.getByText("CodeGate Guardian")).toBeVisible();
    await expect(
      page.getByText("Scan your repository for AI tool security risks")
    ).toBeVisible();
  });
  ...
  test("can type in the input field", async ({ page }) => {
    ...
    await expect(input).toHaveAttribute(
      "placeholder",
      "Paste a config or repository URL to scan..."
    );
  ...
  test("suggested actions are scan-oriented on empty chat", async ({ page }) => {
    ...
    await expect(
      page.getByText("Scan https://github.com/owner/repo")
    ).toBeVisible();
  });
